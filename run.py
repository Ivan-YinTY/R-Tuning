...
if __name__ == '__main__':
    ...
    # continue learing
    parser.add_argument('--origin_root_path', type=str, default=None, help='Path to original dataset')
    parser.add_argument('--origin_data_path', type=str, default=None, help='Filename of original dataset')
    ...
    parser.add_argument('--use_lwf', action='store_true', help='Use Learning without Forgetting (LwF) for finetuning', default=False)

    # distillation
    parser.add_argument('--temperature', type=float, default=3.0, help='Temperature for distillation')
    parser.add_argument('--distillation_lambda', type=float, default=0.2, help='Weight for distillation loss')
    
    # synthetic data
    parser.add_argument('--use_synthetic', action='store_true', help='Use synthetic data for training', default=False)
    parser.add_argument('--synthetic_size', type=int, default=2000, help='Number of synthetic samples')
    ...
    safe_print("========== Experiment Arguments ==========")
    ...
    if args.is_training:
        for ii in range(args.itr):
            ...
            if (not args.ddp or args.local_rank == 0) and args.origin_root_path and args.origin_data_path:
                exp.test_on_original_data(setting, test=1)
            if not args.ddp and not args.dp:
                exp.test(setting)
                if args.origin_root_path and args.origin_data_path:
                    print("CL运行在非多卡训练模式")
                    exp.test_on_original_data(setting, test=1)
