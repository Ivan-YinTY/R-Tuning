...
def visual(true, preds=None, name='./pic/test.pdf', rank=None):
    """
    Results visualization
    Only works on rank=0 or when rank is None.
    Adds rank info to filename if provided.
    """
    # 若指定了 rank，则加入到文件名中避免冲突
    if rank is not None:
        name_split = os.path.splitext(name)
        name = f"{name_split[0]}_rank{rank}{name_split[1]}"
