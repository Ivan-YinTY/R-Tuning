...
class UnivariateDatasetBenchmark(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if not isinstance(seq_x_mark, torch.Tensor):
            seq_x_mark = torch.tensor(seq_x_mark, dtype=torch.float32)
        if not isinstance(seq_y_mark, torch.Tensor):
...
class CrowdFlowDataset(Dataset):
    def __init__(self, root_path, flag='train', size=None, data_path='crowdflow1_noisy.csv', scale=True, nonautoregressive=False, test_flag='T', subset_rand_ratio=1.0):
        self.seq_len = size[0]
        self.input_token_len = size[1]
        self.output_token_len = size[2]
        self.flag = flag
    ...
    def __read_data__(self):
        ...
        if 'y' in df_raw.columns:
            data = df_raw['y'].values.astype(np.float32).reshape(-1, 1)
        else:
            # 自动选择所有非字符串列（排除时间戳等）
            data = df_raw.select_dtypes(include=[np.number]).values.astype(np.float32)
        data_len = len(data)
        num_train = int(data_len * 0.7)
        num_test = int(data_len * 0.2)
        num_vali = data_len - num_train - num_test
